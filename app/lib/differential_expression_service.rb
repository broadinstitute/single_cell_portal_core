...
class DifferentialExpressionService
  ...
  #   - +annotation_name+  (String) => Name of requested annotation
  #   - +annotation_scope+ (String) => Scope of requested annotation ('study' or 'cluster')
  #   - +annotation_label+ (String) => comparison label in pairwise calculation (optional)
  #   - +reference_label+  (String) => reference label in pariwise calculation (optional)
  #
  # * *raises*
  #   - (ArgumentError) => if requested parameters do not validate
  def self.validate_annotation(
    cluster_group, study, annotation_name, annotation_scope, annotation_label: nil, reference_label: nil
  )
    pairwise = annotation_label.present? || reference_label.present?
    validate_pairwise(annotation_label, reference_label) if pairwise
    ...
    if !pairwise && cells_by_label.keys.count < 2
      ...
      missing = {
        "#{reference_label}" => cells_by_label[reference_label].count,
        "#{annotation_label}" => cells_by_label[annotation_label].count
      }.keep_if { |_, c| c < 2 }
      raise ArgumentError,
  ...
  # ensure both annotation_label and reference_label are provided for pairwise calculations
  def self.validate_pairwise(annotation_label, reference_label)
    missing = { annotation_label:, reference_label: }.keep_if { |_, v| v.blank? }
